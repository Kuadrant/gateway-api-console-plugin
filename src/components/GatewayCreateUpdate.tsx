...
const GatewayCreateUpdate: React.FC<GatewayCreateUpdateProps> = ({
  gatewayResource,
  formValidation,
  gatewayModel,
  ns,
  ...
  console.log(update);
  console.log(
    `gatewayResource.metadata.creationTimestamp: ${gatewayResource.metadata.creationTimestamp}`,
  );
  ...
  const handleCreateUpdate = async () => {
    ...
    if (update && (!gatewayResource.metadata?.uid || !gatewayResource.metadata?.resourceVersion)) {
      ...
      setErrorAlertMsg(
        t('Missing required metadata fields (uid, resourceVersion) for gateway update'),
      );
      return;
    }
      ...
      <>
        <div>
          {errorAlertMsg != '' && (
            <AlertGroup>
              <Alert
                title={t(update ? `Error updating gateway` : `Error creating gateway`)}
        ...
        <div style={{ marginLeft: '16px', display: 'flex', gap: '8px' }}>
          ...
          <Button onClick={() => history.push(`/dashboards`)}>{t(`Cancel`)}</Button>
        </div>
      </>
    );
  } else {
